...
export function handlePause(event: Pause): void {
  ...
  if (market === null) {
    ...
    market.totalXDAIClaimed = ZERO_BD;
    market.winRate = HUNDRED_BD;
    market.averageXDAI = ZERO_BD;
    market.netXDAI = ZERO_BD;
    market.save();
  }
  market.epoch = event.params.epoch.toString();
  market.paused = true;
  ...
  if (round.closePrice) {
    if (round.closePrice.equals(round.lockPrice as BigDecimal)) {
      ...
      if (market === null) {
        ...
        market.totalXDAITreasury = market.totalXDAITreasury.plus(round.totalAmount);
        market.netXDAI = market.netXDAI.plus(round.totalAmount);
        market.save();
      }
...
export function handleBetBull(event: BetBull): void {
  ...
  market.averageXDAI = market.totalXDAI.div(market.totalBets.toBigDecimal());
  market.netXDAI = market.netXDAI.minus(event.params.amount.divDecimal(EIGHTEEN_BD));
  market.save();
  ...
  if (user === null) {
    ...
    user.totalXDAIBull = ZERO_BD;
    user.totalXDAIBear = ZERO_BD;
    user.totalBetsClaimed = ZERO_BI;
    user.totalXDAIClaimed = ZERO_BD;
    user.winRate = HUNDRED_BD;
    user.averageXDAI = ZERO_BD;
    user.netXDAI = ZERO_BD;
  ...
  user.totalXDAIBull = user.totalXDAIBull.plus(event.params.amount.divDecimal(EIGHTEEN_BD));
  if (user.totalBets.gt(ZERO_BI)) {
    user.winRate = user.totalBetsClaimed.divDecimal(user.totalBets.toBigDecimal()).times(HUNDRED_BD);
    user.averageXDAI = user.totalXDAI.div(user.totalBets.toBigDecimal());
  }
  user.netXDAI = user.netXDAI.minus(event.params.amount.divDecimal(EIGHTEEN_BD));
  user.save();
...
export function handleBetBear(event: BetBear): void {
  ...
  user.totalXDAI = user.totalXDAI.plus(event.params.amount.divDecimal(EIGHTEEN_BD));
  user.totalXDAIBear = user.totalXDAIBear.plus(event.params.amount.divDecimal(EIGHTEEN_BD));
  if (user.totalBets.gt(ZERO_BI)) {
    user.winRate = user.totalBetsClaimed.divDecimal(user.totalBets.toBigDecimal()).times(HUNDRED_BD);
    user.averageXDAI = user.totalXDAI.div(user.totalBets.toBigDecimal());
  }
...
export function handleClaim(event: Claim): void {
  ...
  if (bet === null) {
    ...
    bet.claimedXDAI = event.params.amount.divDecimal(EIGHTEEN_BD);
    if (bet.amount.gt(ZERO_BD)) {
      bet.claimedNetXDAI = event.params.amount.divDecimal(EIGHTEEN_BD).minus(bet.amount);
    }
    bet.updatedAt = event.block.timestamp;
    bet.save();
  }
  ...
  user.totalXDAIClaimed = user.totalXDAIClaimed.plus(event.params.amount.divDecimal(EIGHTEEN_BD));
  if (user.totalBets.gt(ZERO_BI)) {
    user.winRate = user.totalBetsClaimed.divDecimal(user.totalBets.toBigDecimal()).times(HUNDRED_BD);
  }
  user.netXDAI = user.netXDAI.plus(event.params.amount.divDecimal(EIGHTEEN_BD));
  user.save();
  ...
  market.totalXDAIClaimed = market.totalXDAIClaimed.plus(event.params.amount.divDecimal(EIGHTEEN_BD));
  market.winRate = market.totalBetsClaimed.divDecimal(market.totalBets.toBigDecimal()).times(HUNDRED_BD);
  market.netXDAI = market.netXDAI.plus(event.params.amount.divDecimal(EIGHTEEN_BD));
  market.save();
}
