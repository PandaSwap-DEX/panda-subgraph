...
import { ACC_PANDA_PRECISION, BOOST_PRECISION, BI_ONE, BI_ZERO } from "../utils";
...
export function handleUpdatePool(event: UpdatePool): void {
  log.info("[MasterChefV2] Update Pool {} {} {} {}", [
    ...
    event.params.accPandaPerShare.toString(),
  ]);

  const masterChef = getOrCreateMasterChef(event.block);
  const pool = getOrCreatePool(event.params.pid, event.block);

  pool.accPandaPerShare = event.params.accPandaPerShare;
...
export function handleDeposit(event: Deposit): void {
  ...
  user.rewardDebt = user.amount
    .times(multiplier)
    .div(BOOST_PRECISION)
    .times(pool.accPandaPerShare)
    .div(ACC_PANDA_PRECISION);
...
export function handleUpdatePandaRate(event: UpdatePandaRate): void {
  log.info("[MasterChefV2] Update Panda Rate {} {} {}", [
  ...
  masterChef.pandaRateToBurn = event.params.burnRate;
  masterChef.pandaRateToRegularFarm = event.params.regularFarmRate;
  masterChef.pandaRateToSpecialFarm = event.params.specialFarmRate;
