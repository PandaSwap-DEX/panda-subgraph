...
import { BD_1E18, BD_ZERO, BI_ONE, DAR_XDAI, TRACKED_TOKEN_XDAI_PAIRS, TRACKED_TOKEN_USDC_PAIRS } from "./utils";
...
export function handleSwap(event: Swap): void {
  ...
  if (event.address.toHex() == DAR_XDAI) {
    user.darVolumeUSD = user.darVolumeUSD.plus(volumeUSD);
    user.darVolumeXDAI = user.darVolumeXDAI.plus(volumeXDAI);
  }
